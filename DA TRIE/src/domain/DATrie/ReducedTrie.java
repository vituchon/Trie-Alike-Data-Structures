 ...
 */
public class ReducedTrie implements TrieTree
{

    ReducedTrieNode[] nodes;
    StringBuffer tail;
    int tailPos;
    int[] charCodes;
    int maxNode;
    ...
    public ReducedTrie()
    {
        ...
        this.nodes = new ReducedTrieNode[INIT_CAPACITY];
        for (int i = 0; i < INIT_CAPACITY; i++)
        {
            this.nodes[i] = new ReducedTrieNode();
        }
    ...
    public long space()
    {
        ...
        bytes += this.nodes.length * ReducedTrieNode.space(); // tama��o de los contenidos de los punteros [nodes]
    ...
    public long optimalSpace()
    {
        ...
        bytes += this.maxNode * ReducedTrieNode.space(); // tama��o de los contenidos de los punteros [nodes]
        bytes += tail.length(); // tama��o de la cola
        return bytes;
    }
    ...
    private void insert_2(String str, int i, int curNode, int curBase)
    {
        ...
        int temp = -curBase, q = 1, newNode = curNode, lastNode = curNode;
        ...
        {
            //insert_2_bis (sep[1],curNode,this.tailPos);

            this.nodes[curNode].isTerminal = true;
            this.nodes[curNode].base = -tailPos;

            sep[1] = escapeString(sep[1]);
            this.tail.insert(this.tailPos, sep[1] + END_CHAR);
            this.tailPos += sep[1].length() + 1;
        ...
        {
            //insert_2_bis(sep[0],curNode, temp);
            this.nodes[curNode].isTerminal = true;
            this.nodes[curNode].base = -temp;

            sep[0] = escapeString(sep[0]);
            this.tail.replace(temp, temp + sep[0].length() + 1, sep[0] + END_CHAR);
        }

    }

    private void insert_2_bis(String str, int curNode, int tempBase)
    {
        ...
        // se establece como hoja el nuevo nodo
    ...
    private void insert_3(String str, int i, int curNode, int colisionNode)
    {
        ...
        for (char ch : chars_checknode_fail.toCharArray())
        {
            ...
            this.nodes[tempNode1].check = ReducedTrieNode.UNDEFINED;
    ...
    private void do_back_delete(int node)
    {
        ...
        while (node != ROOT_NODE && !this.nodes[node].isTerminal && !have_children(node))
        {
            ...
            this.nodes[node].check = ReducedTrieNode.UNDEFINED;
            this.nodes[node].base = 0;
            this.nodes[node].isTerminal = false;
            node = last;
        }
    ...
    private int x_check(String list)
    {
        ...
        do
        {
            ...
            while (i < chars.length && find == true)
            {
                ...
                find = (this.nodes[t].check == ReducedTrieNode.UNDEFINED);
                i++;
            }
            if (find == false)
                q++;
    ...
    private void increment_size(int inc)
    {
        ...
        ReducedTrieNode[] temp = this.nodes;
        this.nodes = new ReducedTrieNode[lenght + inc];
    ...
    private void copy_strings(int node, StringBuffer buffer)
    {
        ...
        for (int i = 0; i < this.charCodes.length; i++)
        {
            ...
            else if (this.nodes[node].check != ReducedTrieNode.UNDEFINED)
            {
    ...
    private void list_nodes_rec(StringBuffer buf, int node)
    {
        ...
        Vector<Integer> nodos = new Vector<Integer>(10);
        buf.append("De Nodo: " + node + ". isTerminal = " + this.nodes[node].isTerminal + "\n");
        for (int i = 0; i < this.charCodes.length; i++)
        {
            if (this.nodes[node].base > 0)
            {
            ...
            {
                buf.append("   Nodo Hoja: " + this.tailRetrieve(-this.nodes[node].base) + "\n");
                i = this.charCodes.length;
            }
    ...
    private void get_strings_rec(String common_prefix, Vector<String> res, int node)
    {
        for (int j = 0; j < this.charCodes.length; j++)
        {
            ...
            {
                res.add(common_prefix + this.tailRetrieve(-this.nodes[node].base));
    ...
    private void initCharCodes()
    {
        ...
        charCodes['á'] = j++; // a mayuscula
        charCodes['é'] = j++; // e mayuscula
        charCodes['í'] = j++; // i mayuscula
        charCodes['ó'] = j++; // o mayuscula
        charCodes['ú'] = j++; // u mayuscula
